...
| Tests (fast) | `venv/bin/python -m pytest tests/unit tests/security -q` (6 tests) |
...
- **Entry point:** `POST /api/generate` with body `{"topic": str}` → `MainContentFlow(topic).execute()` (`backend/flows/main_flow.py`). Also `GET /` and `GET /health`.
- **Pipeline:** input guardrail → researcher → writer → critic → fact-checker → revision loop (`max_revisions=3`) → social-format generation. Returns `blog_post`, `linkedin_post`, `x_post`, `detailed_overview`, `metrics`, `status`.
...
- `guardrails/hallucination_guardrail.py` — `check_hallucination()`, deterministic phrase scan producing `hallucination_rate` / `trust_score`; wired.
- `guardrails/output_guardrail.py` — stub, never called.
...
## Known Gaps (documented behavior — don't "fix" silently)

- The revision loop never fires: `run_critic` hardcodes `approved=True` and `run_fact_checker` passes clean text, so the editor stage is effectively unreachable in practice.
- Unused scaffolding: `crews/`, `tasks/`, `rag/`, `tracer.trace_execution`, `output_guardrail`, tavily tool, `SocialMediaCampaign` model, redis in docker-compose, `VECTOR_DB_URL`.
- `backend/api/dependencies.py` `get_settings()` exists but routes import the `settings` singleton directly.
